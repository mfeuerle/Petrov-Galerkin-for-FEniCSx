...
from pgfenicsx import setup_boundary_meshtags, dirichletbc, assemble_system
...
nx = 15
nt = np.ceil((I[1]-I[0])/((Omega[1]-Omega[0])/(nx+1))).astype('int')
...
bdry_tagged = setup_boundary_meshtags(msh, bdry)
ds = ufl.Measure("ds", domain=msh, subdomain_data=bdry_tagged)

U = fem.functionspace(msh, ("Lagrange", 1))
V = fem.functionspace(msh, ("Lagrange", 1))

bcs = [ dirichletbc(U, u0, bdry_tagged.find(bdry_prt.t0)),
        dirichletbc(U, g0, bdry_tagged.find(bdry_prt.gamma)),
        dirichletbc(V, 0, bdry_tagged.find(bdry_prt.T)),
        dirichletbc(V, 0, bdry_tagged.find(bdry_prt.gamma))]
...
A1,l1 = assemble_system((A,l), bcs, include_bc=True)
u1 = spsolve(A1, l1)

A2,l2,trial_bc = assemble_system((A,l), bcs, include_bc=False)
u2 = np.empty((trial_bc.ndofs,))
u2[trial_bc.free_dofs] = spsolve(A2, l2)
u2[trial_bc.fixed_dofs] = trial_bc.values

u_exact_discrete = fem.Function(U)
u_exact_discrete.interpolate(u_exact)

print(np.linalg.norm(u1 - u_exact_discrete.x.array, ord=np.inf))
print(np.linalg.norm(u2 - u_exact_discrete.x.array, ord=np.inf))
...
try:
    ...
    plotter.subplot(0, 0)
    plot_pyvista(u_exact_discrete.x.array, "u_exact", plotter)
    plotter.subplot(0, 1)
    plot_pyvista(u1, "u1", plotter)
    plotter.subplot(0, 2)
    plot_pyvista(u2, "u2", plotter)
    # plotter.subplot(1, 1)
